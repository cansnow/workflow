	...
	<div class="meg-sheet" @scroll="l_scroll" :class="{ 'meg-sheet-formatpainter': curAction == 'formatpainter' }">
		<div class="sheet-content table" :style="{ top: scrollTop + 'px', left: scrollLeft + 'px' }">
			...
			<div class="tr tr1" v-if="showHead">
				<div class="td td-empty"><div class="table-empty" /></div>
				<!--行头-->
				<div class="td table-colhead-wrap">
					<Heads direction="column" ref="colHead" :scroll="scrollLeft" :fixed="fixedColumnIndex" :compen="plusWidth" :flows="blockFlows.rb.columnFlows"></Heads>
				</div>
			</div>
			...
			<div class="tr tr2">
				...
				<div v-if="showHead" class="td table-rowhead-wrap">
					<Heads direction="row" ref="rowHead" :scroll="scrollTop" :fixed="fixedRowIndex" :compen="plusHeight" :flows="blockFlows.rb.rowFlows"></Heads>
				</div>

				<!--数据区-->
				<div class="grid-content" @click="gridContentClick">
					<!--上边数据区-->
					...
					<div class="grid-bottom">
						...
						<!--右下-->
						<div class="grid-rb">
							<Block key="rb" :rowFlows="blockFlows.rb.rowFlows" :columnFlows="blockFlows.rb.columnFlows" :plusHeight="plusHeight" :plusWidth="plusWidth" />
					...
					<div v-for="(style, index) in resizeLineStyle" :key="index" :class="resizeClass" :style="style"></div>
				</div>
			</div>
		</div>
		...
		<Context-Menu v-model="contextMenuState" @click-item="clickItem" :items="curMenuItems" :style="contextMenuPos"></Context-Menu>
	</div>
</template>
<script>
...
export default {
	...
	watch: {
		...
		rowCount(rowCount) {
			this.$emit('update',{rowCount:rowCount});
		},
		columnCount(columnCount) {
			this.$emit('update',{columnCount:columnCount});
		},
		styles(styles) {
			this.$emit('update',{styles:parseStyle1(styles)});
		},
		RCStyles(styles) {
			this.$emit('update',{styles:parseStyle1(styles)});
		},
		...
		merges(merges) {
			this.$emit('update',{merges:Object.values(merges)});
		},
		columns(columns) {
